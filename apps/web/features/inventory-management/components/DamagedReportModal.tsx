...
export function DamagedReportModal({
  open,
  onClose,
  initialSummary,
  onSuccess,
}: DamagedReportModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onSuccess(
        `Laporan barang rusak untuk ${cartItems.length} produk berhasil dikirim.`
      );
      onClose();
    } catch (err) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Laporkan Barang Rusak" size="2xl">
      <form onSubmit={handleSubmit} className="flex flex-col h-[75vh] max-h-[750px]">
        ...
        <div className="shrink-0 space-y-4 pb-4 border-b border-surface-100">
          <div className="bg-amber-50 border border-amber-100 rounded-xl p-3 flex justify-between items-center">
            <div className="flex gap-3 items-center">
              ...
              <p className="text-sm text-amber-900 leading-relaxed font-medium">
                Laporan masuk sebagai request pending owner.
              </p>
            </div>
            <span className="rounded-full bg-danger-50 px-2.5 py-1 text-xs font-black text-danger-700 whitespace-nowrap">
          ...
          <div className="relative search-container z-20">
            <label className="mb-1.5 block text-xs font-bold text-surface-600">
              Cari Produk
            </label>
        ...
        <div className="flex-1 overflow-y-auto py-4 min-h-[200px] flex flex-col">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-bold text-surface-900 flex items-center gap-2">
              <PackageMinus className="h-4 w-4 text-surface-400" />
              Keranjang Barang Rusak
            </h3>
            {cartItems.length > 0 && (
          ...
          {cartItems.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center text-surface-400 bg-surface-50/50 rounded-xl border-2 border-dashed border-surface-200">
              <PackageMinus className="h-12 w-12 opacity-20 mb-3" strokeWidth={1.5} />
                ...
                >
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <span className="text-xs text-surface-500">
                        Stok: <span className="font-semibold text-surface-700">{item.stock} {item.unit}</span>
                      </span>
                    </div>
                  </div>
        ...
        <div className="shrink-0 pt-4 border-t border-surface-100 space-y-3 bg-white mt-auto">
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full resize-none rounded-xl border-2 border-surface-200 px-3 py-2 text-sm outline-none focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10 transition-all bg-surface-50 focus:bg-white text-surface-900 placeholder:text-surface-400"
              placeholder="Keterangan detail kerusakan..."
            />
          </div>
