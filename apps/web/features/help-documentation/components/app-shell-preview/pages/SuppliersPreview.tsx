...
function ShoppingPanel({ ctx }: { ctx: PreviewContext }) {
  const requestOpen = ctx.state === "suppliers-request-open";
  const approveOpen = ctx.state === "suppliers-approve-open";
  return <section className="mt-5 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center justify-between"><div><h2 className="text-base font-black">Permohonan Belanja</h2><p className="mt-1 text-sm text-slate-500">Buat permohonan dan periksa dampak stok sebelum disetujui.</p></div>{requestOpen ? <span className="inline-flex items-center gap-2 rounded-xl bg-brand-600 px-4 py-2 text-xs font-bold text-white"><Plus className="h-4 w-4" />Buat Permohonan Belanja</span> : <PreviewButton ctx={ctx} target="suppliers-create-request" tone="brand" icon={Plus}>Buat Permohonan Belanja</PreviewButton>}</div><div className="mt-5 overflow-hidden rounded-xl border"><div className="grid grid-cols-[190px_1fr_180px_150px] bg-slate-50 p-3 text-xs font-bold"><span>No. Permohonan</span><span>Supplier / Kebutuhan</span><span>Status</span><span>Aksi</span></div>{['DB-20260710-004','DB-20260709-011','DB-20260708-009'].map((id,index) => <div key={id} className="grid grid-cols-[190px_1fr_180px_150px] border-t p-4 text-sm"><strong>{id}</strong><span>{index ? 'Kemasan dan tinta' : 'Kertas dan bahan finishing'}</span><span className="text-amber-700">Diajukan</span>{index === 0 ? approveOpen ? <span className="font-bold text-brand-700">Setujui Permohonan</span> : <GuideTarget ctx={ctx} target="suppliers-approve" className="font-bold text-brand-700">Setujui Permohonan</GuideTarget> : <span>Detail</span>}</div>)}</div></section>;
}

function RequestModal({ ctx }: { ctx: PreviewContext }) {
  return <PreviewModal ctx={ctx} target="suppliers-create-request" title="Buat Permohonan Belanja" width="max-w-4xl"><PreviewField label="Supplier Tujuan *" value="CV Sumber Rezeki" /><GuideTarget ctx={ctx} target="suppliers-add-products" className="mt-4 rounded-xl border p-4"><p className="text-sm font-black">Cari & Tambah Produk</p><div className="mt-3 grid grid-cols-[1fr_180px] gap-3"><span className="rounded-xl bg-slate-50 p-3 text-sm text-slate-500">Cari kertas, tinta, atau kemasan...</span><GuideTarget ctx={ctx} target="suppliers-request-quantity" className="rounded-xl border p-3 text-sm font-bold">Kebutuhan: 12 · Stok Bersama</GuideTarget></div><p className="mt-3 rounded-xl bg-cyan-50 p-3 text-xs font-bold text-cyan-700">Live preview: stok 100 → 112</p></GuideTarget></PreviewModal>;
}
