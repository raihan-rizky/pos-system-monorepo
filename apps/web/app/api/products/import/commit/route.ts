...
const commitSchema = z.object({
  ...
  decisions: z.record(z.string(), z.enum(["create", "update", "skip"])).default({}),
...
export async function POST(request: Request) {
  try {
    ...
    const { rows, decisions, createMissingCategories } = commitSchema.parse(await request.json());
    ...
    const duplicateSkus = Array.from(
      activeRows.reduce((counts, row) => counts.set(row.sku, (counts.get(row.sku) ?? 0) + 1), new Map<string, number>()),
    ).filter(([, count]) => count > 1).map(([sku]) => sku);

    if (duplicateSkus.length > 0) {
      return NextResponse.json(
        { message: "Import contains duplicate SKUs. Mark extra rows as skip.", duplicateSkus },
        { status: 400 },
      );
    }
    ...
    const result = await db.$transaction(async (tx: Prisma.TransactionClient) => {
      ...
      const existingProducts = await tx.product.findMany({ where: { storeId, sku: { in: skus } } });
      ...
      const missingCategories = Array.from(new Set(rows.map((row) => row.category).filter((name) => !categoryByName.has(name.toLowerCase()))));
      ...
      for (const categoryName of missingCategories) {
        const category = await tx.category.create({
          data: { name: categoryName, icon: "Package", color: "#64748b", order: categories.length + categoryByName.size + 1 },
        });
      ...
      for (const row of rows) {
        ...
        const decision = existing
          ...
          : decisions[String(row.rowNumber)] ?? decisions[row.sku] ?? "create";
        ...
        if (decision === "skip") {
          ...
          await tx.batchOperationItem.create({
            data: {
              ...
              sku: row.sku,
              action: "SKIP",
              beforeSnapshot: existing ? productSnapshot(existing) as unknown as Prisma.InputJsonValue : undefined,
              afterSnapshot: existing ? productSnapshot(existing) as unknown as Prisma.InputJsonValue : undefined,
            },
          });
          continue;
        }
        ...
        if (existing) {
          ...
          const log = row.stock === 0 ? null : await tx.inventoryLog.create({
            data: {
          ...
          await tx.batchOperationItem.create({
            data: {
              ...
              afterSnapshot: productSnapshot(created) as unknown as Prisma.InputJsonValue,
              inventoryLogId: log?.id,
            },
          });
        }
    ...
    if (error instanceof Error) {
      if (error.message.startsWith("MISSING_CATEGORIES:")) {
        return NextResponse.json({ message: "Missing category confirmation", missingCategories: error.message.replace("MISSING_CATEGORIES:", "").split(", ") }, { status: 400 });
      }
      if (error.message.startsWith("ROW_DECISION_REQUIRED:")) {
        return NextResponse.json({ message: "Existing SKU rows require update or skip decisions", rowNumber: Number(error.message.replace("ROW_DECISION_REQUIRED:", "")) }, { status: 400 });
      }
    }

    console.error("Failed to commit product import:", error);
    return NextResponse.json({ message: "Failed to commit product import" }, { status: 500 });
  }
}
