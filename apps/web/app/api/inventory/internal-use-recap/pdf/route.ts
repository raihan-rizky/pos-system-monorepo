...
import { buildInternalUseRecapPdf } from "@/features/internal-use-recap/helpers/internal-use-recap-pdf";
import { resolveInternalUsePeriodRange, todayJakartaIsoDate } from "@/features/internal-use-recap/helpers/period";

const log = getLogger("api:inventory:internal-use-recap:pdf");
...
export async function GET(request: NextRequest) {
  try {
    ...
    const pdf = await buildInternalUseRecapPdf(recap);
    const filename = `rekap-pemakaian-internal-${recap.period}-${recap.range.start}-${recap.range.end}.pdf`;
    ...
    log.error("Failed to export internal use recap PDF:", error);
    return apiError("Failed to export internal use recap PDF", 500, {
      code: "InternalError",
    });
  }
}
