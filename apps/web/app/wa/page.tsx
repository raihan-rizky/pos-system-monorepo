...
import { useWaContacts, useWaMessages, useSendMessage } from "@/hooks/useWaChat";
import { formatDate } from "@/lib/utils";

/**
 * Resolve image_url: if it's a wa_media:ID reference, proxy through our API route.
      ...
      <div className="flex-1 flex ml-0 md:ml-[72px] bg-white border-l border-surface-200">
        ...
        <div className={`w-full md:w-[350px] flex-shrink-0 flex-col border-r border-surface-200 ${selectedChatId ? "hidden md:flex" : "flex"}`}>
          ...
          <div className="flex-1 overflow-y-auto">
            {contactsLoading ? (
              <div className="p-8 flex justify-center text-sm font-medium text-surface-400">Loading chats...</div>
            ) : contacts.length === 0 ? (
              <div className="p-8 flex justify-center text-sm text-surface-400">Belum ada riwayat percakapan dari WA.</div>
            ) : (
                  ...
                  >
                    ...
                    <div className="flex gap-2">
                       {contact.role === "assistant" && (
                         <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#0c98e9" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="flex-shrink-0 mt-0.5"><polyline points="20 6 9 17 4 12"></polyline></svg>
                       )}
                      <p className={`text-sm truncate flex-1 ${contact.role === "assistant" ? "text-surface-500" : "text-surface-600 font-medium"}`}>
                        {contact.content}
                      </p>
                    </div>
                  </li>
        ...
        <div className={`flex-1 flex-col bg-[#EFEAE2] relative ${!selectedChatId ? "hidden md:flex" : "flex"}`}>
          ...
          {!selectedChatId ? (
            <div className="relative z-10 flex-1 flex flex-col items-center justify-center text-surface-500 bg-surface-50">
              ...
              <h2 className="text-xl font-bold text-surface-600">POS WhatsApp Coexistence</h2>
              <p className="mt-2 text-sm text-surface-400 text-center max-w-sm">Pilih pelanggan dari daftar di sebelah kiri untuk melihat riwayat percakapan dan mengambil alih obrolan bot.</p>
            </div>
          ) : (
            <>
              {/* Chat Header */}
              ...
              <div className="relative z-10 flex-1 overflow-y-auto p-4 md:p-6 space-y-3 md:space-y-4">
                {messagesLoading ? (
                  <div className="text-center text-surface-500 text-sm p-4 font-medium backdrop-blur-sm bg-white/50 rounded-lg inline-block mx-auto mx-auto block w-max">Memuat rekapan obrolan...</div>
                ) : (
                  messages.map((msg, idx) => {
                    const isAssistant = msg.role === "assistant";
                    return (
                      <div key={msg.id || idx} className={`flex flex-col ${isAssistant ? "items-end" : "items-start"}`}>
                        <div 
                        ...
                        >
                          {(() => {
                            ...
                            return imgSrc ? (
                              ...
                              <div className="flex items-center gap-2 text-xs opacity-70 mb-2 px-3 py-2 rounded-lg bg-black/10">
                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                  <rect x="3" y="3" width="18" height="18" rx="2" ry="2"/>
                                  <circle cx="8.5" cy="8.5" r="1.5"/>
                                  <polyline points="21 15 16 10 5 21"/>
                                </svg>
                                <span>Gambar (media WA)</span>
                              </div>
                            ) : null;
                          })()}
                          <div className={`wa-markdown leading-relaxed ${
                            isAssistant ? "wa-markdown--light" : "wa-markdown--dark"
                          }`}>
                            <ReactMarkdown
                              components={{
                                ...
                                a: ({ href, children }) => (
                                  <a href={href} target="_blank" rel="noopener noreferrer" className={`underline ${isAssistant ? "text-blue-100" : "text-brand-600"}`}>{children}</a>
                                ),
                                code: ({ children }) => (
                                  <code className={`px-1 py-0.5 rounded text-[13px] font-mono ${isAssistant ? "bg-white/20" : "bg-surface-100"}`}>{children}</code>
                                ),
                                pre: ({ children }) => (
                                  <pre className={`p-2 rounded-lg my-1 overflow-x-auto text-[13px] ${isAssistant ? "bg-white/15" : "bg-surface-50 border border-surface-200"}`}>{children}</pre>
                                ),
                                ...
                                blockquote: ({ children }) => (
                                  <blockquote className={`border-l-2 pl-2 my-1 ${isAssistant ? "border-white/40 text-white/80" : "border-surface-300 text-surface-600"}`}>{children}</blockquote>
                                ),
                              }}
                            >
                              {msg.content}
                          ...
                          <div className={`text-[10px] mt-1 font-medium ${isAssistant ? "text-brand-100 text-right" : "text-surface-400 text-left"}`}>
              ...
              <div className="relative z-20 p-3 md:p-4 bg-surface-50 border-t border-surface-200">
                <form onSubmit={handleSend} className="flex gap-2.5 max-w-4xl mx-auto">
