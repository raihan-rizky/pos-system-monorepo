      ...
      <div className="relative flex flex-col w-full max-w-md max-h-[90vh] bg-white rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 border border-surface-100">
        ...
        <div className="shrink-0 px-6 pt-5 pb-4 border-b border-surface-100 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-surface-900 leading-tight">Setujui Permintaan</h2>
              <p className="text-xs text-surface-500 mt-0.5">Konfirmasi dan proses pembayaran transaksi</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 px-6 py-5 space-y-4 overflow-y-auto min-h-0 text-sm">
          ...
          <div className="p-3.5 rounded-xl bg-amber-50/50 border border-amber-200/60 text-xs text-amber-800 flex items-start gap-2.5">
            ...
            <span className="leading-relaxed font-medium">
              Cocokkan invoice cetak dari sales dengan transaksi pending ini sebelum memproses persetujuan.
            </span>
          </div>
          ...
          <div className="rounded-xl border border-surface-200 bg-white p-4 space-y-3.5 shadow-sm">
            <div className="flex justify-between items-center pb-2.5 border-b border-surface-100">
              <div>
                <span className="text-[10px] font-bold text-surface-400 uppercase tracking-wider block">No. Invoice</span>
                <span className="text-sm font-bold text-surface-900 mt-0.5 block">{tx.invoiceNumber}</span>
              </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <span className="text-xs font-semibold text-surface-500 block mb-0.5">Pelanggan</span>
                <span className="font-bold text-surface-900 block truncate" title={tx.customerName || "Pelanggan Umum"}>
                  {tx.customerName || "Pelanggan Umum"}
                </span>
              </div>
              <div className="text-right">
                <span className="text-xs font-semibold text-surface-500 block mb-0.5">Total Tagihan</span>
          ...
          {!isAlreadyLunas && (
            <label className="flex items-center justify-between p-3.5 rounded-xl border border-surface-200 bg-surface-50 hover:bg-surface-100/50 cursor-pointer transition-all duration-200">
              <div className="flex items-center gap-3">
                ...
                />
                <div>
                  <span className="text-sm font-bold text-surface-955 block">Bayar Nanti (Tempo)</span>
                  <span className="text-xs text-surface-500 block mt-0.5">Tandai transaksi sebagai jatuh tempo (DP)</span>
                </div>
              </div>
            </label>
          )}
          ...
          {/* Payment Section */}
          <div className={`transition-all duration-200 ${isPayLater ? "opacity-50 pointer-events-none" : ""}`}>
            {hasMultiPayment ? (
              <div className="rounded-xl border border-surface-200 p-4 space-y-3 bg-surface-50/50">
                <div className="text-xs font-bold text-surface-500 uppercase tracking-wider pb-2 border-b border-surface-100 flex items-center justify-between">
                  ...
                  <span className="text-brand-600 normal-case font-semibold">Metode Multi-payment</span>
                </div>
                <div className="space-y-2.5">
                  {tx.payments!.map((p, idx) => (
                    <div key={idx} className="flex justify-between items-center text-sm">
                      ...
                      <span className="font-bold text-surface-900">{formatRupiah(Number(p.amount))}</span>
                    </div>
                  ))}
                </div>
                <div className="pt-3 border-t border-surface-100 flex justify-between items-center text-sm">
                  <span className="font-semibold text-surface-600">Total Dibayar</span>
                  <span className="font-bold text-brand-700">{formatRupiah(multiPaymentTotal)}</span>
                </div>
              </div>
            ) : (
                ...
                <div>
                  ...
                  <div className="relative">
                    <select
                      ...
                      className="w-full pl-3.5 pr-10 py-2.5 rounded-xl border border-surface-200 bg-white text-sm font-semibold text-surface-900 cursor-pointer
                        focus:outline-none focus:ring-2 focus:ring-brand-500/30 focus:border-brand-500 transition-all appearance-none"
                      disabled={isPayLater}
                    >
                ...
                <div>
                  ...
                  <div className="relative rounded-xl shadow-sm">
                    <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                      <span className="text-surface-400 text-sm font-semibold">Rp</span>
                    </div>
                    <input
                      type="number"
                      ...
                      disabled={isPayLater}
                      placeholder="0"
                      className="w-full pl-10 pr-4 py-2.5 bg-white border border-surface-200 rounded-xl text-sm font-bold text-surface-900
                        focus:outline-none focus:ring-2 focus:ring-brand-500/30 focus:border-brand-500 disabled:bg-surface-50 disabled:text-surface-400 transition-all"
                    />
                  </div>
                </div>
              </div>
        ...
        <div className="shrink-0 px-6 py-4 border-t border-surface-100 bg-white flex items-center gap-3">
          ...
          <button
            ...
            className="flex-1 py-2.5 rounded-xl bg-brand-600 text-white text-sm font-semibold
              hover:bg-brand-700 active:scale-[0.98] transition-all disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer flex items-center justify-center gap-1.5"
          >
            {approveTx.isPending ? (
              <>
